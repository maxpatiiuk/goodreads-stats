 ...
 *
 * Inspired by https://github.com/kristopolous/Porter-Stemmer/
 */

import type { IR, RA } from '../../utils/types';
import { mappedFind, sortFunction, sum } from '../../utils/utils';
...
const replace =
  ...
  (word: string): string => {
    const mapped = mappedFind(
      Object.entries(replacements).sort(
        sortFunction(([suffix]) => suffix.length, true)
      ),
      ([suffix, replacement]) =>
...
const steps: RA<(word: string) => string> = [
  // Step 1a
  replace({
    sses: 'ss',
    ies: 'i',
